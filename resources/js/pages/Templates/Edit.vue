                    ...
                    <div class="rounded-lg border border-border bg-card p-6 shadow-sm">
                        <div class="grid gap-4">
                            <div>
                                ...
                                <input v-model="form.name" type="text" class="w-full rounded-lg border border-border bg-background px-4 py-2.5 text-sm focus:border-primary focus:outline-none" />
                                <p v-if="form.errors.name" class="mt-1 text-xs text-destructive">{{ form.errors.name }}</p>
                            </div>
                            <div class="grid gap-4 sm:grid-cols-2">
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-foreground">Event Type</label>
                                    <select v-model="form.type" class="w-full rounded-lg border border-border bg-background px-4 py-2.5 text-sm focus:outline-none">
                                ...
                                <div>
                                    ...
                                    <select v-model="form.channel" class="w-full rounded-lg border border-border bg-background px-4 py-2.5 text-sm focus:outline-none">
                            ...
                            <div v-if="form.channel === 'email'">
                                ...
                                <input v-model="form.subject" type="text" class="w-full rounded-lg border border-border bg-background px-4 py-2.5 text-sm focus:outline-none" />
                            </div>
                            <div>
                            ...
                            <div>
                                <div class="mb-1.5 flex items-center justify-between">
                                    ...
                                    <button type="button" :disabled="aiLoading" class="flex items-center gap-1 rounded-lg border border-indigo-200 bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700 transition hover:bg-indigo-100 disabled:opacity-40" @click="aiImprove">
                                        <Loader2 v-if="aiLoading" class="h-3 w-3 animate-spin" /><Sparkles v-else class="h-3 w-3" /> AI Improve
                                    </button>
                                </div>
                                <textarea v-model="form.content" rows="5" class="w-full rounded-lg border border-border bg-background px-4 py-3 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
